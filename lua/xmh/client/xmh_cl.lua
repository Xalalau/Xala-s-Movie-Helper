...
local function HideOrShow(Panel)
    ...
    if checkAdmin() == true and not game.SinglePlayer() then
        xmh_menu = Panel:CheckBox (XMH_LANG[_LANG]["client_menu_hideshow_invisibleall"     ], "xmh_invisibleall_var")
        xmh_menu:SetTooltip       (XMH_LANG[_LANG]["client_menu_hideshow_invisibleall_desc"])
    end
    xmh_menu = Panel:CheckBox (XMH_LANG[_LANG]["client_menu_hideshow_tgun"             ], "xmh_toolgun_var" )
    xmh_menu:SetTooltip       (XMH_LANG[_LANG]["client_menu_hideshow_tgun_desc"        ])
    if checkAdmin() == true then
        xmh_menu = Panel:CheckBox (XMH_LANG[_LANG]["client_menu_hideshow_tgunsound"        ], "xmh_toolgunmute_var" )
        xmh_menu:SetTooltip       (XMH_LANG[_LANG]["client_menu_hideshow_tgunsound_desc"   ])
    end
    xmh_menu = Panel:CheckBox (XMH_LANG[_LANG]["client_menu_hideshow_pgun"             ], "xmh_physgun_var")
    xmh_menu:SetTooltip       (XMH_LANG[_LANG]["client_menu_hideshow_pgun_desc"        ])
    xmh_menu = Panel:CheckBox (XMH_LANG[_LANG]["client_menu_hideshow_errors"           ], "xmh_error_var")
    xmh_menu:SetTooltip       (XMH_LANG[_LANG]["client_menu_hideshow_errors_desc"      ])
    xmh_menu = Panel:CheckBox (XMH_LANG[_LANG]["client_menu_hideshow_misc"             ], "xmh_weapammitem_var")
    xmh_menu:SetTooltip       (XMH_LANG[_LANG]["client_menu_hideshow_misc_desc"        ])
    xmh_menu = Panel:CheckBox (XMH_LANG[_LANG]["client_menu_hideshow_voicen"           ], "xmh_chatvoice_var")
    xmh_menu:SetTooltip       (XMH_LANG[_LANG]["client_menu_hideshow_voicen_desc"      ])
    if checkAdmin() == true then
        if not game.SinglePlayer() then
    ...
    if checkAdmin() == true then
        ...
        xmh_menu:SetTooltip        (XMH_LANG[_LANG]["client_menu_hideshow_corpses_desc"     ])
    end
    xmh_menu = Panel:NumSlider (XMH_LANG[_LANG]["client_menu_hideshow_deathn"           ], "hud_deathnotice_time", 0, 12, 0)
    xmh_menu:SetTooltip        (XMH_LANG[_LANG]["client_menu_hideshow_deathn_desc"      ])
    xmh_menu = Panel:NumSlider (XMH_LANG[_LANG]["client_menu_hideshow_bchat"            ], "hud_saytext_time", 0, 24, 0)
    xmh_menu:SetTooltip        (XMH_LANG[_LANG]["client_menu_hideshow_bchat_desc"       ])
    xmh_menu = Panel:NumSlider (XMH_LANG[_LANG]["client_menu_hideshow_decals"           ], "xmh_decals_var", 1, 5096, 0)
    xmh_menu:SetTooltip        (XMH_LANG[_LANG]["client_menu_hideshow_decals_desc"      ])
    xmh_menu = Panel:NumSlider (XMH_LANG[_LANG]["client_menu_hideshow_detail"           ], "cl_detaildist", 0, 20000, 0)
    xmh_menu:SetTooltip        (XMH_LANG[_LANG]["client_menu_hideshow_detail_desc"      ])
end
...
local function Flashlight(Panel)
    ...
    --xmh_menu = Panel:CheckBox  (XMH_LANG[_LANG]["client_menu_flashlight_area"           ], "r_flashlightdrawfrustum") -- Stopped working (14/11/16)
...
local function Shadows(Panel)
    ...
    xmh_menu:SetTooltip        (XMH_LANG[_LANG]["client_menu_shadows_notes"          ])
